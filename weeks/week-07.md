...
**Manual references:** Instrument View (manual p.16), Instrument Modulation View (manual p.18), Wavsynth (manual p.50), Macrosynth (manual p.52), Sampler (manual p.54), FM Synth (manual p.58), Hypersynth (manual p.60).
...
- **Modulation (`MOD1`–`MOD4`)** — each slot is an `AHD ENV`, `ADSR ENV`, `DRUM ENV` (purpose-built for percussion), `LFO`, `TRIG ENV`, or `TRACKING` (maps note/velocity to a parameter), pointed at any `DEST` (manual p.20).
...
- [ ] **Sub bass** — `EG-Bass-Sub`
	...
	- **Build it:**
		1. TYPE `Wavsynth`, SHAPE `SINE` (or `Macrosynth` `SINE TRIANGLE` for a touch more body, manual p.76) → a sine has almost no harmonics, so it stays out of the way of every other sound.
...
- [ ] **Acid bass** — `EG-Bass-Acid`
	...
	- **Build it:**
		1. TYPE `Macrosynth`, SHAPE a saw model (`SAW SQUARE` or `SAW SYNC`, manual p.76) → a saw is harmonically rich, giving the resonant filter something to chew on.
		...
		5. Table: this is where the 303 glide and pitch moves live. Two commands handle pitch, and they do different jobs:
			- `PIT XX` — offsets the note pitch in **semitones** (manual p.75): `PIT 0C` throws the row up an octave, and the upper half of the range (`80`–`FF`) offsets *downward*. It's a stepped jump, not a slide, and it's scale-quantized (manual p.28) so the offset snaps to the track's scale. Drop a `PIT` row or two for an octave jump or a quick accent-wail into the next note.
			- `PSL XX` — **pitch slide** (portamento) is the command that actually makes the smooth 303 glide *between* notes. The value is in *ticks* (manual p.71) — higher = slower glide. The community-standard acid glide is around `PSL 06`–`07`, with the instrument set to **legato (no retrigger)** so consecutive notes slide into each other instead of re-plucking.
...
- [ ] **FM bass** — `EG-Bass-FM`
	...
	- **Build it (from a blank patch — no presets needed):**
		...
		1. TYPE `FM Synth` (manual p.58). Leave `ALGO` on the first one, `A>B>C>D` (`00`) — a single 4-operator chain. The arrows mean "modulates," so the **last operator `D` is the carrier** (the one you actually hear) and **`C` is its modulator**.
		2. **Carrier — operator `D`:** shape `SIN`, `RATIO 1.00`, `LEV` full. This is the fundamental: `RATIO 1.00` plays the note you press, `2.00` an octave up, `0.50` an octave down (manual p.58).
		...
		6. **Make it evolve — the move that turns a static FM tone into a *bass*.** In the Modulation view (manual p.18) set `MOD1` to `ADSR`: ATK=00 DEC=30 SUS=00. Then in operator **`C`**'s `MOD` slot, route `MOD1` → `LEV`. C's level now snaps up on the attack and decays back toward `00` — the `LEV` destination sweeps the operator from `00` up to its set level (manual p.58) → the FM brightness *blooms, then fades to a near-pure tone*. That bright "pock" over a clean tail is the signature DX/FM bass shape. (Raise `SUS` if you want the harmonics to stay through held notes.)
...
- [ ] **Pluck lead** — `EG-Lead-Pluck`
	...
	- **Build it:**
		1. TYPE `Macrosynth` `PLUCKED` model (manual p.77) for instant plucked-string physics, or any saw model / `Wavsynth` `SAW` if you want to shape it by hand → `PLUCKED` does the work for you; a saw gives full manual control.
...
- [ ] **Pad** — `EG-Pad`
	...
	- **Build it:**
		1. TYPE `Macrosynth`, a soft/smooth model (e.g. `MORPH`, manual p.76) → pads want a mellow source, not a buzzy saw.
...
- [ ] **Stab/chord** — `EG-Stab`
	...
	- **Build it:**
		1. TYPE `Macrosynth` (play a chord across tracks) or `Hypersynth` (manual p.60) to generate the chord from a single note → Hypersynth saves you voicing chords by hand (see Thread 3 Option B).
...
- [ ] **Kick** — `EG-Drum-Kick`
	...
	- **Build it:**
		...
		2. If synthesized: `MOD1` `DRUM ENV` → `PITCH`, a fast high→low sweep → the pitch drop *is* the "thump." The `DRUM ENV` is purpose-built for percussion (manual p.20) — sharper than an ADSR for this.
...
- [ ] **Snare** — `EG-Drum-Snare`
	...
	- **Build it:**
		1. **Source (pick one) — a snare is noise + a tuned shell, so you want both parts:**
			...
			- One `FM Synth` → the efficient single-instrument route, consistent with the kick above. Set `ALGO` to the last one (additive mode: all four operators run in parallel to the output, manual p.58). Give one operator a maxed `FB` for the noise body — cranking feedback all the way *is* how M8 FM generates noise — and a second operator a `SINE`/`CLICK` tuned via `RATIO` to sit ~200Hz for the tonal snap; balance the two with `LEV`. M8's own factory clap is this exact trick (maxed-out FM noise).
		2. `MOD1` `DRUM ENV` (or `ADSR`) → `VOLUME`, short decay, no sustain → a snare is a fast burst, not a sustained tone.
		3. `MOD2` `TRACKING` → `VOLUME`, SRC `VELOCITY` → ghost notes at low velocity, hard backbeats at high velocity = a human-feeling snare (manual p.20).
...
**Firmware note:** on 6.5.x, `TIC:FD` retriggers on *note* changes rather than velocity changes, which makes a velocity-mapped instrument behave erratically. Firmware 6.6.0 fixes this, along with a related `TIC:FC`/`TIC:FE` retrigger bug. If these recipes fight you, check your firmware version before blaming the recipe — see [Firmware Reference](../reference/firmware.md).

**Where to find it (this is the part people miss):** there is no separate "TIC type" menu. `TABLE TIC` is a single numeric field in Instrument View — the one you normally leave at `01` (one tick per table row), `02` for two ticks, and so on (manual p.23). The special modes live at the *top* of that same field: dial the value all the way up past the tick-count range to hit `FC` = octave map, `FD` = velocity map, `FE` = note map, `FF` = 200 Hz (manual p.24). So "set it to `TICFD`" really means "scroll the `TABLE TIC` value up to `FD`" — the M8 doesn't show these as named options, just as the high hex values of the field. (The `TICxx` spelling is the FX-command form: dropping `TICFD` in a phrase or table FX column does the same thing and overrides the instrument's field — p.23.)
...
**Why `FIN`, not `PIT`, on a tuned drum:** `PIT` moves in whole **semitones** and is **scale-quantized** (manual p.28), so on a snare (whose tuned shell is recognizable) a hard hit jumps to an obviously different pitch and just sounds *detuned*. `FIN` offsets by less than a semitone (manual p.75), giving lifelike variation without the "wrong note" effect. Carry most of the hard-vs-soft difference in **tone** (`CUTOFF`, a `REV`/`DEL` send) rather than pitch. On a non-tuned source (noise hat, FX) big `PIT` jumps are fair game.

**On table volume — the `V` column is *relative*, not a boost:** the table's `V` (volume) column is *multiplied* by the phrase's `V` (manual p.24), so it can only scale the incoming level down/up proportionally — it can't add a fixed amount on its own. To actually *offset* the level on a row, use the **`VOL XX` FX command** ("offset the instrument volume", manual p.75) in one of the three FX columns. That's why the steps above put a volume *boost* in an FX column rather than in the `V` column.
...
- [ ] Hypersynth respects the project's Scale (manual p.28). Set a scale on the track via `SCA` FX command, or globally via `SCG`.
